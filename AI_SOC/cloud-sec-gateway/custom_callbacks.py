...
def get_user_role_from_ldap(username: str) -> str:
    ...
    try:
        ...
        server = Server(os.getenv('LDAP_SERVER', 'ldap://127.0.0.1:389'), get_info=ALL, connect_timeout=1)
        conn = Connection(server, os.getenv('LDAP_USER', 'cn=admin,dc=casb,dc=local'), os.getenv('LDAP_PASSWORD', 'CHANGE_ME'), auto_bind=True)
        conn.search(os.getenv('LDAP_BASE_DN', 'ou=People,dc=casb,dc=local'), f'(uid={username})', attributes=['gidNumber'])
        if conn.entries:
